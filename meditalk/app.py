...
# Ensure `st.session_state` attributes are initialized
if 'is_recording' not in st.session_state:
    st.session_state.is_recording = False  # Initialize to False
if 'prompt_text' not in st.session_state:
    st.session_state.prompt_text = None
if 'chat_text' not in st.session_state:
    st.session_state.chat_text = None
if 'user_lang' not in st.session_state:
    st.session_state.user_lang = 'en'  # Default to English

# Define button callbacks
def callback_record():
    if not st.session_state.is_recording:  # Ensure it doesn't trigger multiple times
...
with st.container():
    ...
    with right:
        ...
        rec_button = st.button(
            ...
            disabled=st.session_state.is_recording)  # Disable button when recording is happening
...
with st.container():
    ...
    if st.session_state.chat_text:
        lines = st.session_state.chat_text.split('.')
        for line in lines:
